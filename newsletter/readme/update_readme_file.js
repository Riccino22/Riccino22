...
module.exports = function updateReadmeFile(readmePath, svgDir) {
  return async function (cards) {
    ...
    const rels = await Promise.all(
      cards.map(async (svgContent, i) => {
        const rank = i + 1
        const filename = `story_${rank}.svg`
        await fs.writeFile(path.join(svgDir, filename), svgContent, 'utf8')
        return { rank, rel: path.relative(path.dirname(readmePath), path.join(svgDir, filename)).replace(/\\/g, '/') }
      })
    )
    ...
    for (let i = 0; i < rels.length; i += 2) {
      ...
      if (right) {
        rows.push(`<tr>\n<td width="50%"><img src="${left.rel}" alt="Story ${left.rank}" width="100%"/></td>\n<td width="50%"><img src="${right.rel}" alt="Story ${right.rank}" width="100%"/></td>\n</tr>`)
      } else {
        rows.push(`<tr>\n<td width="50%"><img src="${left.rel}" alt="Story ${left.rank}" width="100%"/></td>\n<td width="50%"></td>\n</tr>`)
      }
    }
